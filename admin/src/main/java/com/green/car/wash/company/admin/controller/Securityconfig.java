...
import com.green.car.wash.company.admin.config.JwtRequestFilter;
...
public class Securityconfig extends WebSecurityConfigurerAdapter
{
	...
	protected void configure(HttpSecurity http) throws Exception
	{
		/*
		 * http.csrf().disable()
		 * .authorizeRequests().antMatchers("/admins/register","/admins/login").
		 * permitAll() .antMatchers("/user/**").hasAnyRole("CUSTOMER","ADMIN")
		...
		http.csrf().disable()
		  .authorizeRequests().antMatchers("/v3/api-docs/**","swagger-ui/**","/admins/register","/admins/login","/admins/**")
